...
画法：左侧输入，中间小霓或怪机器处理，右侧输出，橙色箭头表达主流向。

### 系统局部

适合：信息来源、过滤器、数据库、渲染器、agent 系统局部。

画法：只画 3-5 个核心模块，小霓参与其中一个关键动作。
...
画法：一层层盒子，不要正式金字塔；小霓在旁边搬砖或搭建。

### 地图路线

适合：从想法到上线、用户路径、内容承接路径、学习路线。

画法：一条弯曲路径，少量节点，小霓牵线或走路。
...
3. 让小霓承担动作：不是站旁边，而是卡在机器里、拉错线、守门、搬运、修补、称重、扶梯子、记录、把东西塞进某个怪装置。
...
不要默认打开或复刻 `assets/examples/` 下的案例图片（见 `assets/examples/README.md`）。这些图片只用于风格校准，例如线条密度、留白、颜色克制、小霓气质。
...
同类主题也要换新隐喻。例如“承接路径”不一定画路线，可以画小霓把内容尾巴接到门把手；“一鱼多吃”不一定画鱼，可以画小霓把一个纸团压成几种形状。
